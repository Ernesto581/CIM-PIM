...
export default function Wizard() {
  ...
  const persist = async (contenido, umlCode) => {
    const etapa = {
      id: stage.id,
      nombre: stage.nombre,
      nivel: stage.nivel,
    ...
    const updated = { ...proyecto, etapas: upsertEtapa(proyecto.etapas, etapa) };
    const { data } = await client.put(`${API.proyectos}/${id}`, updated);
    setProyecto(data);
    return data;
  };
  ...
  const saveDescripcion = async () => {
    setSaving(true);
    try {
      const { data } = await client.put(`${API.proyectos}/${id}`, { ...proyecto, descripcion });
      setProyecto(data);
      setSnack('Descripción guardada');
    } catch (e) {
      setSnack('Error al guardar la descripción');
    } finally {
      setSaving(false);
    }
  };
  ...
  const generate = async () => {
    ...
    try {
      const { data } = await client.post(API.generate, {
        requisitos: requisitosInput(),
        etapa: stage.id
      });
      const content = data.data.markdown || '';
      const umlCode = data.data.uml || '';
      setMarkdown(content);
      setUml(umlCode);
      await persist(content, umlCode);
      setSnack(`${stage.nombre} generado con IA`);
    } catch (e) {
      setSnack(e.response?.data?.error || 'Error al generar con la IA');
    } finally {
      setLoading(false);
    }
  };

  if (!proyecto || stages.length === 0) {
    return (
      <Layout>
